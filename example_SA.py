...
def update(frame_number):
    points["position"] = results.traj_x[frame_number]
    scatter.set_offsets(points["position"])
    ax.set_title('Alpine 1 function, Temperature: ' + str(round(results.traj_t[frame_number],2)))
    return scatter, 

anim = FuncAnimation(fig, update, interval=10, frames=range(len(results.traj_x)), repeat_delay=2000)
plt.show()
